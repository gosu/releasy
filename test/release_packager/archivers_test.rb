...
[
    [:"7z",    ReleasePackager::Archivers::SevenZip, %[7z a -mmt -bd -t7z "f.7z" "f"]],
    [:tar_bz2, ReleasePackager::Archivers::TarBzip2, %[7z a -so -mmt -bd -ttar "f.tar" "f" | 7z a -si -bd -tbzip2 "f.tar.bz2"]],
    [:tar_gz,  ReleasePackager::Archivers::TarGzip,  %[7z a -so -mmt -bd -ttar "f.tar" "f" | 7z a -si -bd -tgzip "f.tar.gz"]],
    [:zip,     ReleasePackager::Archivers::Zip,      %[7z a -mmt -bd -tzip "f.zip" "f"]],
